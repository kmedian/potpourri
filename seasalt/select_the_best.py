...
def select_the_best(res, n_best=5, lowest=False):  
    ...
    summary = res[['params', 'cvratio', 'rank_test_score', 'mean_test_score', 'std_test_score']]
...
def select_the_best(res, n_best=5, lowest=False):  
    res = pd.DataFrame(res.cv_results_)
    res = res.sort_values(by='mean_test_score', ascending=lowest)[:n_best]  # pick the N best
    res['cvratio'] = res['mean_test_score'] / res['std_test_score']  # compute best vs CV result std
    res = res.sort_values(by='cvratio', ascending=lowest)  # pick the best cv/std of the N best
    bestparams = res['params'][res.index[0]]  # best params
    summary = res[['params', 'cvratio', 'rank_test_score', 'mean_test_score', 'std_test_score']]
    return bestparams, summary
"""
